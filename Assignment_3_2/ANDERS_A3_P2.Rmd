...
m1 <- glm(Diagnosis ~ scaled_pitch_variability, data = df, family = 'binomial')
summary(m1)
...
#making the recipe/model on training data
rec <- df_par_train %>% 
  recipe(Diagnosis ~ pitch_variability, data = df_par_train)

# making predicter numeric and scaling it
rec_steps <- rec %>% 
  step_scale(all_numeric()) %>% 
  step_center(all_numeric())

#?????????????????
prepped_recipe <- prep(rec_steps, training = df_par_train, retain = T)
df_par_train_n <- juice(prepped_recipe) #%>% select(-c(Study, ID, .partitions))
df_par_test_n <- bake(rec, new_data = df_par_test, all_predictors()) #%>% select(-c(Study, ID, .partitions))
