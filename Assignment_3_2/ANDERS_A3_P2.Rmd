...
```{r}
################################################# TIDYMODELLING ##############################################
#Note that this workflow doesnt contain any cross-validation. It is simply partioning the data, preprocessing the features, training the model, and then validating the model.
...
#execute preprocessing on training and test data, removing coloumn that dont predict
my_training <- juice(my_recipe) %>% select(-c(Study, New_id, Participant))

my_testing <- my_recipe %>% 
  bake(testing(df_split)) %>% 
  select(-c(Study, New_id, Participant))
