...
# making a common column called study with only numerics
duration$soundname <- as.character(duration$soundname)
duration$Study <- substr(duration$soundname, 6,6)
duration$Study <- as.factor(duration$Study)

duration$Subject <- substr(duration$soundname, 10,12)

# Finally the pitch data
# common column of study number
#pitch_data$Study <- as.factor(pitch_data$Study)
#pitch_data$Subject <- str_extract(pitch_data$Subject, '\\d')
pitch_data$Trial <- str_replace_all(pitch_data$Trial, '[[:punct:]]', '')

# Now we merge them

#data <- merge(demo, duration, by = 'Subject', all = T)
#data <- merge(data, pitch_data, by = 'Subject', all = T)
