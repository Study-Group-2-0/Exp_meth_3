...
read_pitch <- function(filename) {
    files = path_file(path = '/Pitch') %>%
              for (i in files)
                    Study = substr(files, 1,6) 
                    Diagnosis = substr(files, 7,8)
                    Subject = substr(files, 9, 12)
                    Trial = substr(files, 13,15) %>% 
                data1 = data.frame(c(
                    Study, Diagnosis, Subject, Trial)) %>% 
                data2 = read_delim(filename) %>% 
                data = cbind(data1, data2, by = row)
    # load data
...
test = path_file(path = "Pitch/Study1D0S101T1_f0.txt")

test_split = strsplit(test, split = "")

Study = substr(test, 1,6)
Diagnosis = substr(test, 7,8)
Subject = substr(test, 9, 12)
Trial = substr(test, 13,15)
