...
duration <- duration %>% mutate(
    ...
    Subject = as.numeric(Subject),
    Trial = substr(duration$soundname, 14,15),
    #Trial = str_replace_all(duration$Trial, '[[:punct:]]', ''),
...
# adding 1000 to study 5, 6 & 7 to avoid a mess
pitch_data$Subject <- ifelse(pitch_data$Study > 4, pitch_data$Subject + 1000, pitch_data$Subject)
duration$Subject <- ifelse(duration$Study > 4, duration$Subject + 1000, duration$Subject)
demo$Subject <- ifelse(demo$Study > 4, demo$Subject + 1000, demo$Subject)

demo$Study <- as.factor(demo$Study)
demo$Subject <- as.factor(demo$Subject)
duration$Study <- as.factor(duration$Study)
duration$Subject <- as.factor(duration$Subject)
pitch_data$Study <- as.factor(pitch_data$Study)
pitch_data$Subject <- as.factor(pitch_data$Subject)

# Now we merge them

df <- merge(demo, pitch_data, by = c('Subject', 'Study', 'Diagnosis'), all = T)
df1 <- merge(df, duration, by = c('Subject','Study', 'Diagnosis', 'Trial'), all = T)
