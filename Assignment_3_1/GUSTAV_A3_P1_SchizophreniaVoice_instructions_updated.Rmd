...
```{r}
path_name <- str_c("Pitch/","Study6D1S044T22.txt")
data <- read_tsv(path_name)

#Extracting variables from the file name
vars <- str_match(path_name, "Study(\\d+)D(\\d)S(\\d+)T(\\d+)")
...
```{r}
pacman::p_load(tidyverse, EnvStats, DescTools)
...
# Now we save them
write_csv(df1, "FINAL_DATA_1.csv")
```

## Now we need to describe our sample
```{r}
#Loading the final dataset
df <- read.csv("FINAL_DATA_1.csv") %>% 
