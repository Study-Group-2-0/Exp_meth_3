...
```{r}
numchi1 <-length(unique(demo_train$Child.ID))
numchi2 <- length(unique(LU_train$SUBJ))
numchi3 <-length(unique(token_train$SUBJ))

##### CHANGING COLOUMN NAMES ############

#changing to SUBJ in demo_train
colnames(demo_train)[1:2] <- c("SUBJ", "VISIT")


########### CHANGINS VISIT COLOUMN ###########

#making into characters
LU_train$VISIT <- as.character(LU_train$VISIT)

#doing substring
LU_train$VISIT <- substring(LU_train$VISIT[1:352], 6, 6)

#making into factor again
LU_train$VISIT <- as.factor(LU_train$VISIT)

#making VISIT on demo_train into factor
demo_train$VISIT <- as.factor(demo_train$VISIT)


#making into characters
token_train$VISIT <- as.character(token_train$VISIT)

#doing substring
token_train$VISIT <- substring(token_train$VISIT[1:352], 6, 6)
...
token_train$SUBJ <- as.factor(token_train$SUBJ)
```


```{r}
##################### REMOVING THE EXTRA PARTICIPANTS IN DEMO_TRAIN ##########

# LU and token datasets have the same participants
levels(token_train$SUBJ) == levels(LU_train$SUBJ)

#Converting to tibbles
demo_train <- as_tibble(demo_train)
LU_train <- as_tibble(LU_train)

#semi_join function Compare two data.frames to find the rows in data.frame    1 that are not present in data.frame 2
demo_train <- semi_join(LU_train, demo_train)

#Now all the 3 dataframes have the same participants
levels(b$SUBJ) == levels(token_train$SUBJ)
levels(b$SUBJ) == levels(LU_train$SUBJ)

#They also have the same values in VISIT
levels(b$VISIT) == levels(token_train$VISIT)
levels(b$VISIT) == levels(LU_train$VISIT)
...
#for demotrain
var1 <- c("SUBJ", "VISIT", "Diagnosis", "Ethnicity", "Gender", "Age", "ADOS", "MullenRaw", "ExpressiveLangRaw", "Socialization")

# for LU_train
var2 <- c("SUBJ", "VISIT", "MOT_MLU", "CHI_MLU")

#for token_train
var3 <- c("SUBJ", "VISIT", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")


#making the subsettes dataframes
demo_train <- select(demo_train, var1)
LU_train <- select(LU_train, var2)
token_train <- select(token_train, var3)
...
```{r}
#variables to only take VISIT1 values from
newvar <- c("SUBJ", "VISIT", "ADOS", "MullenRaw", "ExpressiveLangRaw", "Socialization")

# filter chooses which rows to keep, select chooses the coloumns to keep
dff <- select(filter(df, df$VISIT==1), newvar)

#renaming coloumns
newnames <- c("SUBJ", "VISIT", "ADOS1", "MullenRaw1", "ExpressiveLangRaw1", "Socialization1")
colnames(dff)[1:6] <- newnames
...
```{r}
#Same as the chunk before just using ifelse
...
```{r}
#1
bonus1 <- df %>% group_by(SUBJ) %>% summarize(mean(CHI_MLU))
filter(bonus1, bonus1[2] > 2.7)


#2
bonus2 <- df %>% filter(df$VISIT==1) %>% filter(CHI_MLU<1.5)


#3
#how many sessions each subject has
bonus3 <- df %>% group_by(SUBJ) %>% summarize(length(SUBJ))

#filtering out subjects with less than 6 sessions
bonus3_1 <- filter(bonus3, bonus3[2] < 6)
...
```{r}
#most amount of words on 6.th visit, cant get descending order to work
bonusarr <- df %>% filter(df$VISIT==6) %>% arrange(tokens_CHI)


bonusarr2 <- df %>% filter(df$VISIT==1) %>% arrange(tokens_CHI)
...
```{r}
#1
bonussel <- select(df, "Diagnosis", "MOT_MLU", "CHI_MLU", "tokens_MOT", "tokens_CHI")
bonussel <- filter(bonussel, Diagnosis == "ASD")

#2
#lets see about that
bonussel2 <- select(bonussel, "Diagnosis", "Diagnosis", "MOT_MLU", "tokens_MOT", "tokens_MOT")
...
```{r}
#1.
dfm <- mutate(df, mean_num_of_words = (mean(df$tokens_MOT) + mean(df$tokens_CHI))/2)

#2.
dfmm <- df %>% group_by(SUBJ) %>% summarize(mean(tokens_CHI))
df <- merge(df, dfmm)

# by summarizing we lose the development that the children has. Maybe normal kids have a stronger language developmelt as they grow. This info is lost when we take a mean. 
