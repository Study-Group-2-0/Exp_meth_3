...
```{r}
#Getting an overview of the different collumns using Janitor package
...
#using the function on all the df
...
#token_train$SUBJ <- str_replace_all(token_train$SUBJ, "[[:punct:]]", "") #ALL THREE WAYS OF SELECTING THE SPECIAL CHARACTERS
...
df <- merge(demo_train, LU_train, by = c("SUBJ","VISIT"), all = TRUE)
df <- merge(df, token_train, by = c("SUBJ","VISIT"), all = TRUE)
```
...
visit1 <- df %>% 
  ...
  select(SUBJ,
         ADOS1 = ADOS,
         MullenRaw1 = MullenRaw,
         ExpressiveLangRaw1 = ExpressiveLangRaw,
...
df <- df %>% 
  mutate(SUBJ = as.factor(as.integer(SUBJ)),
         VISIT = as.factor(VISIT),
         Gender = as.factor(if_else(Gender == 1, "M", "F")),
         Diagnosis = as.factor(if_else(Diagnosis == 1, "ASD", "TD")))
...
df %>% 
  group_by(SUBJ) %>% 
  summarise(LENGTH_VISIT = length(SUBJ)) %>% 
  filter(LENGTH_VISIT == 6)
```
...
df %>% 
  ...
  select(SUBJ, CHI_MLU, tokens_CHI)
