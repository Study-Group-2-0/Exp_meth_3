...
#Checking how many participants in each df
length(unique(demo_train$Child.ID))
length(unique(LU_train$SUBJ))
length(unique(token_train$SUBJ))

#Investigating how many times each child participated
demo_train %>% group_by(Child.ID) %>% summarise(length(Child.ID))
LU_train %>% group_by(SUBJ) %>% summarise(length(SUBJ))
token_train %>% group_by(SUBJ) %>% summarise(length(SUBJ))

######visit column
class(demo_train$Visit)
demo_train$Visit <- as.factor(demo_train$Visit)

#renaming variables
colnames(LU_train)[2] <- "Visit"
colnames(token_train)[2] <- "Visit"

#renaming child ID/subject column
colnames(LU_train)[1] <- "Subject"
colnames(token_train)[1] <- "Subject"
colnames(demo_train)[1] <- 'Subject'


# better way of doing it using tidyverse
LU_train <- LU_train %>% 
  rename('Subject'='SUBJ',
         'Visit'='VISIT')
...
demo_train$Subject<- str_replace_all(demo_train$Subject, "[:punct:]", "")
LU_train$Subject <- str_replace_all(LU_train$Subject, "[:punct:]", "")
token_train$Subject <- str_replace_all(token_train$Subject, "[:punct:]", "")

LU_train$Subject <- as.factor(LU_train$Subject)
demo_train$Subject <- as.factor(demo_train$Subject)
token_train$Subject <- as.factor(token_train$Subject)
...
#variables <- c('Child.ID', 'Visit', 'Diagnosis', 'Ethnicity', 'Gender', 'Age', 'ADOS', 'MullenRaw', 'ExpressiveLangRaw', 'Socialization', 'MOT_MLU', 'CHI_MLU', 'types_MOT', 'types_CHI', 'tokens_MOT', 'tokens_CHI')

#Selecting relevant variables for each df
variables_to_include1 <- c("Subject", "Visit", "Diagnosis","Ethnicity", "Gender", "Age", "ADOS",  "MullenRaw","ExpressiveLangRaw", "Socialization")
variables_to_include2 <- c("Subject", "Visit", "MOT_MLU", "CHI_MLU")
variables_to_include3 <- c("Subject", "Visit", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")

#subsetting relevant variables
demo_train <- select(demo_train, variables_to_include1)
LU_train <- select(LU_train, variables_to_include2)
token_train <- select(token_train, variables_to_include3)  
...
df <- merge(demo_train, LU_train, by = c("Subject","Visit"))
df <- merge(df, token_train, by = c("Subject","Visit"))
...
#creating a new df with only visit 1
new_df <- df %>% filter(Visit == 1)
#adding the relevant variables
new_df <- new_df %>% select(c(Subject, ADOS, MullenRaw, ExpressiveLangRaw, Socialization))

### one liner from Gustav
#Creating new df for visit 1 only including the relevant variables.
#visit1 <- select(filter(df, df$VISIT==1), c("SUBJ","VISIT","ADOS", "MullenRaw","ExpressiveLangRaw","Socialization"))
...
#making a oneliner for the code above
colnames(new_df)[2:5] = c('ADOS1', 'MullenRaw1', 'ExpressiveLangRaw1', 'Socialization1')

#merging the new dataset with the old
df <- merge(df, new_df, by = c("Subject"))
...
#changing diagnosis columns
df$Diagnosis[df$Diagnosis == 'A'] <- 'ASD'
df$Diagnosis[df$Diagnosis == 'B'] <- 'TD'


#Anomymizing the names
df$Subject <- as.integer(df$Subject)
df$Subject <- as.factor(df$Subject)

### ANOTHER way of change the Gender and diagnosis values - from Gustav
#Changing gender and diagnosis names:
#df$Gender <- as.factor(df$Gender)
#levels(df$Gender) <- c("F","M")
#levels(df$Diagnosis) <- c("ASD","TD")

write.csv(df, 'final.data.sigrid.csv')
...
# 1 kids with MLU > 2.7
class(df$Subject)
df_group <- df %>% group_by(Subject) %>% summarise(mean(CHI_MLU))
df_group <- filter(df_group, df_group[2]>2.7)

# 2 kids with mean MLU < 1.5 at visit 1

df_group_2 <- df %>% group_by(Subject) %>% filter(Visit == '1')
df_group_2 <- filter(df_group_2, mean(CHI_MLU) < 1.5)
#easier way of doing it... from gustav
select(filter(df, Visit == '1' & CHI_MLU < 1.5), c('Subject', 'Visit', 'CHI_MLU'))

# 3 kids who have not completed all trials
# all the rows which contain NAs.. not what we wanted
df_3 <- df[!complete.cases(df),]

# 3) Gustav's version
long <- df %>% group_by(Subject) %>% summarise(length(Subject))
long <- filter(long, long[2] < 6)
```
...
df_2_1 <- df %>% select(c('Subject', 'Visit', 'tokens_CHI')) %>% filter(Visit == '6') %>% arrange(desc(tokens_CHI))
# Subject 59 produced the most words on visit 6 with 1294 words
# could also be written like.. doesn't work
#select(arrange(filter(df, Visit == 6), desc(tokens_CHI), c('Subject', 'Visit', 'tokens_chi')))

# 2)
df_2_2 <- df %>% select(c('Subject', 'Visit', 'tokens_CHI')) %>% filter(Visit == '1') %>% arrange(tokens_CHI)
# Subject 61 produced the least amount of words on the 1st visit with 0 words
...
```{r}
# 1)
df_3_1 <- select(filter(df, Diagnosis == 'ASD'), c('Subject', 'Diagnosis', 'CHI_MLU', 'tokens_CHI'))

# 2)
df_3_2 <-select(df, c('Subject', 'Diagnosis', 'Subject'))
# it appears to only select the column once. so nothing happens
...
```{r}
# 1)
df_4_1 <- df %>% group_by(Subject) %>% summarise(mean(tokens_CHI))
df <- merge(df_4_1, df, by = 'Subject')
colnames(df)[2] <- 'tokens_CHI_mean'
