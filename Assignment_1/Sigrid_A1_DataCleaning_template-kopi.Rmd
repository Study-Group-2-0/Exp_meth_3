...
word_data <- read_csv("token_train.csv")
...
colnames(word_data)[2] <- "Visit"

#renaming child ID/subject column
colnames(LU_train)[1] <- "Child.ID"
colnames(word_data)[1] <- "Child.ID"
colnames(demo_train)[1] <- 'Child.ID'
...
LU_train$Child.ID <- str_replace_all(LU_train$Child.ID, "[:punct:]", " ")
word_data$Child.ID <- str_replace_all(word_data$Child.ID, "[:punct:]", " ")

LU_train$Child.ID <- as.factor(LU_train$Child.ID)
demo_train$Child.ID <- as.factor(demo_train$Child.ID)
word_data$Child.ID <- as.factor(word_data$Child.ID)
...
variables_to_include1 <- c("Child.ID", "Visit", "Diagnosis","Ethnicity", "Gender", "Age", "ADOS",  "MullenRaw","ExpressiveLangRaw", "Socialization")
variables_to_include2 <- c("Child.ID", "Visit", "MOT_MLU", "CHI_MLU")
variables_to_include3 <- c("Child.ID", "Visit", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")
...
word_data <- select(word_data, variables_to_include3)
...
df <- merge(demo_train, LU_train, by = c("Child.ID","Visit"))
df <- merge(df, word_data, by = c("Child.ID","Visit"))
