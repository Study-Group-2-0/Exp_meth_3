...
```{r}
# Load the libraries
pacman::p_load(tidyverse, ggplot2, reshape2, gridExtra, groupdata2,radiant.data)
```
...
grid.arrange(plotHR, plotHR_out)
...
grid.arrange(plotHR_out, plotHR_out_scale)
...
grid.arrange(plotHR_out_scale, plotHR_downs)
...
data_preprocess <- function(filename, threshold = 2.5){
  ...
  mutate(
    ...
    ID = paste0(Study, "_", Pair),
    ID_2 = paste0(Study, "_", Pair, "_", Trial),
...
data <- list.files(path = "data_test/", pattern = ".csv") %>% 
...
data <- data %>% 
  mutate(
    new_ID = paste0(ID, "_", Trial),
    new_ID = as.factor(new_ID)
  )

#write to csv
write_csv(data, "data_clean_2.csv")


#in Study 4, time is "Time" and not "time". Fuark.
...
data <- read_csv("data_clean_2.csv")
...
ggplot(data_clean) +
  ...
  geom_path(aes(time, HR2, color = "Red")) + 
  labs(x = "time", y = "Values") + 
  facet_wrap(data_clean$new_ID)
  



#there is something weird with study 4
data_clean$Study <- as.factor(data_clean$Study)


#plotting Study 1-3
data_clean_S123 <- filter(data_clean, Study != 4)

ggplot(data_clean_S123) +
  geom_path(aes(time,HR1, color = "Blue")) + 
  geom_path(aes(time, HR2, color = "Red")) + 
  labs(x = "time", y = "Values") + 
  facet_wrap(data_clean_S123$new_ID)


#plotting Study 4
data_clean_S4 <- filter(data_clean, Study == 4)

ggplot(data_clean_S4) +
  geom_path(aes(time,HR1, color = "Blue")) + 
  geom_path(aes(time, HR2, color = "Red")) + 
  labs(x = "time", y = "Values") + 
  facet_wrap(data_clean_S4$new_ID)
