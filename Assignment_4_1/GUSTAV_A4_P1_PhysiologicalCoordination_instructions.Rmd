...
df <- df %>% 
  mutate(
    ...
    HR1_previous = lag(HR1),
    HR2_previous = lag(HR2),
    Resp1_previous = lag(Resp2),
    Resp2_previous = lag(Resp2),
    
    #Adding the change variable
    HR1_change = HR1 - lag(HR1),
    HR2_change = HR2 - lag(HR2),
    Resp1_change = Resp1 - lag(Resp1),
    Resp2_change = Resp2 - lag(Resp2)
  ) %>% 
  ...
  mutate(
    ...
    Participant = ifelse(variable == "HR1" 
                         | variable == "Resp1" 
                         | variable == "HR1_previous" 
                         | variable == "Resp1_previous" 
                         | variable == "HR1_change" 
                         | variable == "Resp1_change", 
...
# Making all Na's to 0
df[is.na(df)] <- 0

df <- df %>% 
  # Adding the variables for each participant
  mutate(
    HR = HR1 + HR2, 
    Resp = Resp1 + Resp2,
