...
folds = createFolds(unique(data$Subject, k = 6, list = T, returnTrain = F))

hep <- data.frame(trainRMSE = rep(NA, 10),
                  testRMSE = rep(NA, 10),
                  predictive_error = rep(NA,10)) #Adding a new collumn for mean residual
...
bernie %>% 
  ggplot()+
  aes(x = Visit, y = CHI_MLU, color = "blue")+
  geom_point()+
  geom_smooth(method = lm)+
  geom_point(aes(x = Visit, y = TD_average_MLU, color = "red"))+
  geom_smooth(method = lm, aes(x = Visit, y = TD_average_MLU, color = "red"))
