...
RMSE_mean <- data.frame(k_list, trainRMSE_mean, testRMSE_mean)

for (i in k_list){
  df = cross_valid(data, k, model1)
  RMSE_mean[,2] = mean(df$trainRMSE)
  RMSE_mean[,3] = mean(df$testRMSE)
}

  
  
RMSE_mean <- function(df){
i = 1
for (column in df){
  sum = i+ mean(column)
}
