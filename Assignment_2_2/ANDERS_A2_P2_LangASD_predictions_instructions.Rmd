...
m2 <- lmer(CHI_MLU ~ Age + Diagnosis + (Age|Child.ID), df_train)
summary(m2)
```
...
for (fold in folds){
  ...
  model <- lmer(CHI_MLU ~ Age + Diagnosis + (Age|Child.ID), train)
  train$prediction <- fitted(model)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  trainRMSE_s[i] <- rmse(df_train_fit$CHI_MLU, train$prediction)
  testRMSE_s[i] <- rmse(df_train_fit$CHI_MLU, test$prediction)
  i = i + 1
}
...
diff_s <- trainRMSE_s - testRMSE_s
diff_s

#- Report the results and comment on them. 
# it seems like the rmse is quite similar, no big difference between them, small residuals/variance
```


```{r}
######################################### CROSS VALIDATION ON OUR OWN MODEL ###############################
...
for (fold in folds){
  train <- subset(df_train_fit, !(Child.ID %in% fold))
  test <- subset(df_train_fit, (Child.ID %in% fold))
  model <- lmer(CHI_MLU ~ Diagnosis * Age + (Age|Child.ID)+I(Age^2), train)
  train$prediction <- fitted(model)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  trainRMSE[i] <- rmse(df_train_fit$CHI_MLU, train$prediction)
  testRMSE[i] <- rmse(df_train_fit$CHI_MLU, test$prediction)
  i = i + 1
}
...
```{r}
################################ COMPARING CROSS VALIDATION OF SIMPLE AND COMPLEX MODEL #######################################
mean(trainRMSE_s, na.rm = TRUE)
mean(trainRMSE, na.rm = TRUE)

mean(testRMSE_s, na.rm = TRUE)
mean(testRMSE, na.rm = TRUE)

mean(diff_s, na.rm = TRUE)
mean(diff, na.rm = TRUE)

######### simple model has a lower RMSE on test fold
...
trainRMSE_12 <- rep(NA, k)
testRMSE_12 <- rep(NA, k)
...
for (fold in folds){
  ...
  train$prediction <- fitted(model)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  trainRMSE_12[i] <- rmse(df_train_fit$CHI_MLU, train$prediction)
  testRMSE_12[i] <- rmse(df_train_fit$CHI_MLU, test$prediction)
  i = i + 1
}
...
diff_12 <- trainRMSE_12 - testRMSE_12
diff_12
```
...
trainRMSE_18 <- rep(NA, k)
testRMSE_18 <- rep(NA, k)
...
for (fold in folds){
  ...
  train$prediction <- fitted(model)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  trainRMSE_18[i] <- rmse(df_train_fit$CHI_MLU, train$prediction)
  testRMSE_18[i] <- rmse(df_train_fit$CHI_MLU, test$prediction)
  i = i + 1
}
...
diff_18 <- trainRMSE_18 - testRMSE_18
diff_18
```
...
trainRMSE_24 <- rep(NA, k)
testRMSE_24 <- rep(NA, k)
...
for (fold in folds){
  ...
  train$prediction <- fitted(model)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  trainRMSE_24[i] <- rmse(df_train_fit$CHI_MLU, train$prediction)
  testRMSE_24[i] <- rmse(df_train_fit$CHI_MLU, test$prediction)
  i = i + 1
}
...
diff_24 <- trainRMSE_24 - testRMSE_24
diff_24
...
foldingfun <- function(k){
  
folds <- createFolds(unique(df_train_fit$Child.ID), k = k, list = TRUE, returnTrain = FALSE)

trainRMSE_k <- rep(NA, k)
testRMSE_k <- rep(NA, k)
...
for (fold in folds){
  ...
  train$prediction <- fitted(model)
  test$prediction <- predict(model, test, allow.new.levels = TRUE)
  trainRMSE_k[i] <- rmse(df_train_fit$CHI_MLU, train$prediction)
  testRMSE_k[i] <- rmse(df_train_fit$CHI_MLU, test$prediction)
  i = i + 1
}

#add these to the empty df
return (trainRMSE_k)
testRMSE_k

diff_k <- trainRMSE_k - testRMSE_k
diff_k
...
```{r}
############################ PLOTTING RMSE AS FOLDS INCREASE #########################
# Bonus Question 1: What is the effect of changing the number of folds? Can you plot RMSE as a function of number of folds?
#make more rmse for complex model with differenct folds
#combine them into a df

#making all means into vector
rmse_values <- c(mean(trainRMSE, na.rm = TRUE), mean(trainRMSE_12, na.rm = TRUE), mean(trainRMSE_18, na.rm = TRUE), 
                  mean(trainRMSE_24, na.rm = TRUE), mean(testRMSE, na.rm = TRUE), mean(testRMSE_12, na.rm = TRUE),
                 mean(testRMSE_18, na.rm = TRUE), mean(testRMSE_24, na.rm = TRUE))

rmse_vector_tr_or_ta <- c(0,0,0,0,1,1,1,1)

number_of_folds <- c(6,12,18,24,6,12,18,24)

df_rmse_test <- tibble(
  rmse_values,
  rmse_vector_tr_or_ta,
  number_of_folds
)

ggplot(df_rmse_test, aes(x = number_of_folds, y = rmse_values)) + 
  geom_point() + 
  geom_smooth(method = "lm") + 
  facet_wrap(rmse_vector_tr_or_ta)
...
#Whereas we get smaller cross-validated prodictive errors (see plot) at arround 1.1 for 24 folds
...
abs(df_bernie$CHI_MLU-df_bernie$predicted)
