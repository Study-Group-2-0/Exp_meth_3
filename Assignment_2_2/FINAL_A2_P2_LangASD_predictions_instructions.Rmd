...
bernie <- data_test %>% 
  ...
  select(
    Subject, Diagnosis, Visit, CHI_MLU, mean_td, Absolute_dif, Age) 
  


ggplot(bernie, aes(x = Visit, y = CHI_MLU)) + 
  geom_point(color = "blue") + 
  geom_point(aes(y = mean_td)) + 
  geom_smooth(method = lm, aes(color = "Bernie"), formula = y ~ x + I(x^2)) + 
  geom_smooth(method = lm, aes(x = Visit, y = mean_td, color = 'mean_td'), formula = y ~ x + I(x^2)) + 
