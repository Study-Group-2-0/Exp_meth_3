...
pacman::p_load(tidyverse, janitor, purrr, lme4, lmerTest, MuMIn)
...
#another way of getting an overview of the data
distinct(df)
...
# filter visit 1
df1 <- filter(df, VISIT == 1)
df1 %>% split(df1$Diagnosis) %>% map(summary)
...
chi_mlu ~ age + (1+Age|SUBJ)
chi_mlu ~ diagnosis + (1|SUBJ)

```{r ex2, include = FALSE}

#plotting age and chi_mlu as scatter plot
ggplot(df, aes(df$Age, df$CHI_MLU, color = Diagnosis)) + geom_point() + geom_smooth(method=lm)

# subj and mlu boxplit
ggplot(df, aes(as.factor(SUBJ), CHI_MLU, color = Diagnosis)) + geom_boxplot()

# age and chi_mlu scatterplot + facet wrap
ggplot(df, aes(Age, CHI_MLU, color = Diagnosis)) + geom_point() + geom_smooth(method=lm) + facet_wrap(~Diagnosis)

# diagnosis + chi mlu + boxplot
ggplot(df, aes(as.factor(Diagnosis), CHI_MLU, color= Diagnosis)) + geom_boxplot()
...
r.squaredGLMM(m4)
```
*interaction between age and diagnosis - even better with random slope of age*
...
ggplot(df, aes(Age, CHI_MLU, color=Diagnosis)) + geom_point() + geom_smooth(method=loess)

m5 <- lmer(CHI_MLU ~ Diagnosis * Age + (Age|SUBJ) + I(Age^2),df)
summary(m5)
...
```{r}
# putting all the fitted values into a df
#df$SUBJ <- as.factor(df$SUBJ)
#df_no_na_select <- select(df, c(df$SUBJ, df$Age, df$CHI_MLU))

# rows of MLU CHI with NA's
#df[!complete.cases(df[,12]),]
df <- filter(df, is.na(CHI_MLU) == F, is.na(Age) == F)

#creating a column with row number in orignial df
df$X <- seq(1:346)


# getting the fitted values into a dataframe using model 5
fit_model <- data.frame(X = seq(1:346), FIT = fitted(m5))


# saving all fitted values - merging the two data frames into a new one
test <- merge(fit_model, df, by = 'X', all = T)

# plotting the actual versus the fitted data
ggplot(test, aes(Age, CHI_MLU)) + geom_point() + geom_smooth(method = 'lm', color = 'blue') +
  geom_point(aes(Age, FIT, color = 'red')) + geom_smooth(method = 'lm', color = 'red') + labs(x = 'Age', y = 'Child_MLU') + theme(legend.position = 'none')

# how many rows contain N/As
df[!complete.cases(df),]



# number of rows
length(df[!complete.cases(df$CHI_MLU),])
...
Outcome variable: MOT_MLU
Fixed effects: Diagnosis + Age 

Random effects: Age|SUBJ

```{r ex3, include = FALSE}

ggplot(df, aes(Age, MOT_MLU, color = Diagnosis)) + 
  geom_point() + geom_smooth(method=lm) + facet_wrap(~Diagnosis)

ggplot(df, aes(CHI_MLU, MOT_MLU, color = Diagnosis)) + 
  geom_point() + geom_smooth(method=lm) + facet_wrap(~Diagnosis)

p1 <- lmer(MOT_MLU ~ Diagnosis + Age + (1|SUBJ), df)
p2 <- lmer(MOT_MLU ~ Diagnosis + Age + (Age|SUBJ), df)
p3 <- lmer(MOT_MLU ~ Diagnosis * Age + (1|SUBJ), df)
p4 <- lmer(MOT_MLU ~ Diagnosis * Age + (Age|SUBJ), df)
...
# assesing the worth of each of the models
...
r.squaredGLMM(p1)
r.squaredGLMM(p2) #best?
r.squaredGLMM(p3)
r.squaredGLMM(p4)

```

*We built a model to investigate how the MLU of the parent is predicted by the diagnosis and the age of the child..... *
...
#significant
mo1 <- lmer(CHI_MLU ~ df$ExpressiveLangRaw + (1|SUBJ), df)

#significant
mo2 <- lmer(CHI_MLU ~ df$MullenRaw + (1|SUBJ), df)

#significant
mo3 <- lmer(CHI_MLU ~ df$Socialization + (1|SUBJ), df)

#not interesting, too unbalanced
mo4 <- lmer(CHI_MLU ~ df$Gender + (1|SUBJ), df)

#also too unbalanced
mo5 <- lmer(CHI_MLU ~ df$Ethnicity + (1|SUBJ), df)
...
r.squaredGLMM(mo1) #wauw - so verbal IQ is a good predictor of child MLU
r.squaredGLMM(mo2) #also pretty nice
r.squaredGLMM(mo3) #kinda bad compared to the other models
